...
class MySQLOperating(object):
    ...
    def set_InterfaceTableResponse(self,table,value):
        sql = '''UPDATE %s SET status_code = '%s',response = '%s',time = '%s',datetime = '%s'
          WHERE case_id = '%s' ''' %(table,value['status_code'],value['response'],value['time'],value['datetime'],value['case_id'])
        with self.connection:
            cur = self.connection.cursor()
    ...
    #测试用例执行接口写入接口请求记录表
    def insert_InterfaceTableValue(self,table,value):
        sql = '''INSERT INTO  %s (`case_name`, `time`, `status_code`, `datetime`) VALUES ('%s','%s','%s','%s') ''' %(table,value['case_name'],value['time'],str(value['status_code']),value['datetime'])
        with self.connection:
            cur = self.connection.cursor()
            cur.execute('SET NAMES utf8mb4;')
            try:

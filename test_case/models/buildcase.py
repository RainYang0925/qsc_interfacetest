...
class BuildCase(MyTest):
    ...
    def set_db(self,testcase,r,table,body='null'):
        # 获取接口返回时间
        ...
        # 将请求返回数据写入数据库
        value = {'case_id':testcase['case_id'],
                 'status_code':self.code,
                 'response':response,
                 'time':time,
                 'datetime':self.datetime
                 }
        self.db.set_InterfaceTableResponse(table, value)
        ...
        # 将请求返回数据写入接口请求记录表
        value2 = {'case_name':testcase['case_name'],
                  'status_code':self.code,
                  'time':time,
                  'datetime':self.datetime}
        self.db.insert_InterfaceTableValue('interface_record',value2)
    ...
    def execute_case(self,testcase,tablename):
        ...
        if testcase['request_type'] == 'post':
            ...
            # 结果写入数据库
            self.set_db(testcase,r,tablename,body)

        if testcase['request_type'] == 'put':
            # 获取发起请求时间
            self.datetime = time.strftime(ISOTIMEFORMAT, time.localtime(time.time()))
            # 发起请求
            r = requests.put(self.url, data=json.dumps(body), headers=headers)
            # 结果写入数据库
            self.set_db(testcase,r,tablename,body)
        ...
        if testcase['request_type'] == 'patch':
            ...
            self.datetime = time.strftime(ISOTIMEFORMAT, time.localtime(time.time()))
            # 发起请求
            r = requests.patch(self.url, data=json.dumps(body), headers=headers)
            # 结果写入数据库
            self.set_db(testcase,r,tablename,body)
        ...
        if testcase['expected']:
            expected = eval(testcase['expected'],{'false': 'false', 'true': 'true', 'null': 'null'})
            for key,value in expected.items():
                # 正则匹配接口数据的所有key值
                temp = re.compile('\"([^\"]+)\":').findall(str(response))
                # 将所有的key值去重，排序
                response_keys = sorted(set(temp),key = temp.index)
                # 判断预期结果中的key是否在实际返回结果中
                if key in response_keys:
                    actual = self.compare_response(response,key)[0]['value']
                    # 将字典中的None转化为''字符串,注:python中没有null,只有None
                    if not actual:
                        actual = ''
                    # print u'接口返回数据',actual
                    # print u'预期返回数据',value
                    #判断预期结果是否有返回值,'$'代表接口有数据返回，不确认返回的具体数据
                    if value != '$':
                        #判断预期结果与实际结果数据是否一致
                        if actual == value:
                    ...
                    msg = u'接口返回数据中没有'+"<"+key+">"
                    self.db.set_InterfaceTableValue(tablename,testcase['case_id'], 'actual', str(msg))
                    self.assertTrue(actual,msg)

        # 判断反预期结果
        if testcase['not_expected']:
            expected = eval(testcase['not_expected'], {'false': 'false', 'true': 'true', 'null': 'null'})
            for key, value in expected.items():
                # 正则匹配接口数据的所有key值
                temp = re.compile('\"([^\"]+)\":').findall(str(response))
                # 将所有的key值去重，排序
